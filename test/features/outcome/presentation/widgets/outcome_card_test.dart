...
void main() {
  ...
  Widget harness(Widget child, {double width = 250, double ambientTextScale = 1.0}) {
    return MaterialApp(
      builder: (context, builtChild) {
        return MediaQuery(
          data: MediaQuery.of(context).copyWith(textScaler: TextScaler.linear(ambientTextScale)),
          child: builtChild!,
        );
      },
      home: Scaffold(
  ...
  group('REGRESSION: "N/AAman" name-concatenation bug (_StoryBlock._storyText)', () {
        ...
        (tester) async {
          ...
          for (final text in texts) {
            expect(
              text,
              isNot(contains('Aman')),
              reason: 'no rendered text should contain the player name when the card is '
                  'the N/A fallback — found: "$text"',
            );
            expect(
              text,
              isNot(equals('N/AAman')),
              reason: 'the exact garbled-concatenation regression this test pins',
            );
          }
  ...
  group('REGRESSION: individual footer/badge/chip components never overflow '
    ...
    for (final palette in [
      ...
      testWidgets('$label: CardFooter does not overflow at k=1 width', (tester) async {
        await tester.pumpWidget(
          harness(CardFooter(palette: tierPalette, k: 1.0), width: contentWidth),
        );
        await tester.pump();
        expect(tester.takeException(), isNull);
      });

      testWidgets('$label: StoreBadges does not overflow at k=1 width', (tester) async {
        await tester.pumpWidget(
          harness(StoreBadges(color: tierPalette.baseText, k: 1.0), width: contentWidth),
        );
        await tester.pump();
        expect(tester.takeException(), isNull);
      });
      ...
      testWidgets('$label: OutcomeChip does not overflow with the longest '
          'real chip label (Eternal\'s) at k=1, unbounded width', (tester) async {
        await tester.pumpWidget(
          harness(
            OutcomeChip(
