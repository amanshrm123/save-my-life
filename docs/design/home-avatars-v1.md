...
> **Partial correction — see `docs/design/play-loop-v3.md` §7/§10 and `docs/architecture/v6.md` §7.1.** §3.2 below originally recorded the fill-color band thresholds as `>=60`/`>=20`, "adopted as-is" and "architect-confirmed." Architecture v6 rebases them to **`>=40`/`>=20`** so the green band stays reachable once the Play Loop's scoring model becomes attrition-only (reachable in-run life values 50/40/30/20/10/0 — `>=60` would make green unreachable app-wide, including on the Home card below). `avatar_fill_band.dart` itself is a `flutter-developer` code change tracked separately (v6 §8.3) — this callout, and the corrected value in §3.2, exist only to stop this doc's own record from being read as still-current. Everything else in this section (anatomy, fill mechanic, zero-state rule, and §3.5's `redDark`-vs-`red` token call) is unaffected by the rebase and stays exactly as written below.
...
**Rebased — was `>=60`, now `>=40` (architecture v6 §7.1; see `docs/design/play-loop-v3.md` §7).** Current thresholds: **`>=40` → `AppColors.green`, `>=20` → `AppColors.coral`, else → the danger-red token (§3.5)**. The mockup's original 100%/45%/4% three-sample demonstration still lands exactly the same way under the moved boundary (100 and 45 are both `>=40`, so the demonstration needs no re-verification) — only the boundary between green and coral itself moved, from 60 down to 40, and only because the Play Loop's life economy changed (v6), not because these round thresholds read any less clearly at a glance than before.
...
- **Cross-reference:** the numeric threshold this section's `redDark` choice applies to (§3.2) was rebased by architecture v6 §7.1 (`>=40`/`>=20`, was `>=60`/`>=20`) — see the callout at the top of §3 and `docs/design/play-loop-v3.md` §7. The token choice itself (`redDark` for both the Deaths tile and the danger band) is unaffected by exactly where that boundary sits.
...
4. **Fill bands:** manually drive `bestLifePercent` through 4%/45%/100%-equivalent values (or check via debug override) and confirm danger/coral/green bands match §3.2's **rebased** thresholds (`>=40`/`>=20`, not the original `>=60`), using `AppColors.redDark` (not `red`) for the danger band.
